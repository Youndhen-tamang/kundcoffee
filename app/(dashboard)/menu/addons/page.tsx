...
export default function AddonsPage() {
  ...
  const [sortBy, setSortBy] = useState<"name" | "category" | "price" | "type">("name");
  ...
  const handleSortOrderBlur = async (id: string) => {
    ...
    try {
      ...
      if (res?.success) {
        ...
        setAddons(prev => prev.map(a => a.id === id ? { ...a, sortOrder: editingValue } : a));
      }
    } catch (err) {
  ...
  const openEdit = (a: AddOn) => {
    ...
    setPrice(a.price || {});
    setStockConsumption(a.stocks?.map(s => ({ stockId: s.stockId, quantity: s.quantity })) || []);
    setIsPanelOpen(true);
  };
    ...
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">Manage extra toppings and side items</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard title="Available" value={addons.filter(a => a.isAvailable).length} />
        <MetricCard title="Extras" value={addons.filter(a => a.type === "EXTRA").length} />
        <MetricCard title="Sides" value={addons.filter(a => a.type === "ADDON").length} />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-gray-600 whitespace-nowrap">
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((a) => (
                <tr key={a.id} onClick={() => openEdit(a)} className="hover:bg-red-50/50 transition-colors cursor-pointer group">
                  <td className="px-6 py-4" onClick={(e) => handleSortOrderClick(a, e)}>
                    {editingRowId === a.id ? (
                      <input
                        type="number"
                        value={editingValue}
                  ...
                  <td className="px-6 py-4">{categories.find((c) => c.id === a.categoryId)?.name || "-"}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-xs font-medium border ${a.type === "EXTRA" ? "bg-blue-50 text-blue-700 border-blue-100" : "bg-red-50 text-red-700 border-red-100"}`}>
                        {a.type}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-bold text-gray-900">${a.price?.listedPrice || 0}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${a.isAvailable ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                      {a.isAvailable ? "Available" : "No"}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Edit2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <SidePanel isOpen={isPanelOpen} onClose={() => setIsPanelOpen(false)} title={isEditing ? "Edit Add-on" : "New Add-on"}>
        <div className="space-y-8 pb-24">
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:border-red-500 outline-none" value={type} onChange={(e) => setType(e.target.value as any)}>
                ...
                <div className="flex items-center pt-8">
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" checked={isAvailable} onChange={(e) => setIsAvailable(e.target.checked)} className="w-5 h-5 rounded text-red-600 focus:ring-red-500" />
                        <span className="text-sm font-medium text-gray-700">Available</span>
                    </label>
                </div>
            </div>
            <ImageUpload label="Image" value={typeof imageFile === "string" ? imageFile : undefined} onChange={setImageFile} />
          ...
          <section className="space-y-4">
            <h3 className="text-sm font-bold text-gray-900 uppercase border-b pb-2">Stock Consumption</h3>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 bg-white border-t flex items-center gap-3 z-10">
          {isEditing && selectedId && (
            <Button onClick={async () => { if (!confirm(`Delete item "${name}"?`)) return; try { const res = await deleteAddOn(selectedId); if (res?.success) { refresh(); setIsPanelOpen(false); router.refresh(); } } catch (e) { toast.error("Failed to delete"); } }} variant="secondary" className="bg-red-50 text-red-600 border-red-200 hover:bg-red-100"><Trash2 size={18} /></Button>
          )}
          ...
          <Button onClick={handleSubmit} className="flex-1 bg-red-600 text-white shadow-lg" disabled={!name || isSaving || uploading}>
            {isSaving || uploading ? "Saving..." : isEditing ? "Update Item" : "Create Item"}
          </Button>
        </div>
      </SidePanel>
    </div>
