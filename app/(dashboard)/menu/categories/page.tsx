...
export default function CategoriesPage() {
  ...
  const [sortBy, setSortBy] = useState<"name" | "description" | "dishes">("name");
  ...
  useEffect(() => {
    ...
    f = [...f].sort((a, b) => {
      ...
      else { va = a.dishes?.length || 0; vb = b.dishes?.length || 0; }
  ...
  const handleSortOrderBlur = async (id: string) => {
    ...
    try {
      ...
      if (res?.success) {
        ...
        setCategories(prev => prev.map(c => c.id === id ? { ...c, sortOrder: editingValue } : c));
      }
    } catch (err) {
  ...
  const mostDishesCategory = categories.length > 0 
    ? [...categories].sort((a, b) => (b.dishes?.length || 0) - (a.dishes?.length || 0))[0]
    : null;
    ...
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">Manage your menu categories</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard 
            ...
            subValue={mostDishesCategory ? `${mostDishesCategory.dishes?.length || 0} dishes` : ""}
        />
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex items-center justify-between gap-4 bg-white sticky top-0 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <select value={sortDir} onChange={(e) => setSortDir(e.target.value as any)} className="text-sm border border-gray-200 rounded-lg px-3 py-1.5 outline-none">
          ...
          <table className="w-full text-left text-sm text-gray-600 whitespace-nowrap">
            <thead className="bg-slate-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-gray-700">Category Name</th>
                <th className="px-6 py-4 font-semibold text-gray-700">Description</th>
                <th className="px-6 py-4 font-semibold text-gray-700 text-center">Dishes</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((c) => (
                <tr key={c.id} onClick={() => openEdit(c)} className="hover:bg-red-50/50 transition-colors cursor-pointer group">
                  <td className="px-6 py-4" onClick={(e) => handleSortOrderClick(c, e)}>
                    {editingRowId === c.id ? (
                      <input
                        type="number"
                        value={editingValue}
                  ...
                  <td className="px-6 py-4 max-w-xs truncate text-gray-500">
                    {c.description ? c.description.replace(/[#*_]/g, "") : <span className="text-gray-300 italic">No description</span>}
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={(e) => openEdit(c, e)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                        <Edit2 size={16} />
                      </button>
                    </div>
                  </td>
      ...
      <SidePanel isOpen={isPanelOpen} onClose={() => setIsPanelOpen(false)} title={isEditing ? "Edit Category" : "Add New Category"}>
        <div className="space-y-6 pb-20">
          ...
          <ImageUpload label="Category Image" value={typeof imageFile === "string" ? imageFile : undefined} onChange={setImageFile} />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 bg-white border-t border-gray-100 flex items-center gap-3 z-10">
          {isEditing && selectedId && (
            <Button
              onClick={async () => {
                if (!confirm(`Delete category "${name}"? Remove dishes first.`)) return;
                try {
          ...
          <Button onClick={handleSubmit} className="flex-1 bg-red-600 text-white hover:bg-red-700 shadow-md" disabled={!name || uploading}>
