...
import {
  ...
  getCategories, getDishes, getStocks, getSubMenus,
} from "@/services/menu";
...
import { RichTextEditor, PriceForm, StockConsumptionForm } from "@/components/menu/MenuForms";
...
export default function CombosPage() {
  ...
  const [comboItems, setComboItems] = useState<{ dishId: string; quantity: number; unitPrice: number }[]>([]);
  ...
  const refresh = async () => {
    ...
    const [cData, catData, dData, sData, smData] = await Promise.all([
      getCombos(), getCategories(), getDishes(), getStocks(), getSubMenus(),
    ]);
    setCombos(cData);
  ...
  useEffect(() => {
    const totalActual = comboItems.reduce((acc, item) => acc + (item.unitPrice * item.quantity), 0);
  ...
  const handleManualPriceChange = (newPartial: Partial<Price>) => {
    ...
    const discount = Number(newPartial.discountPrice ?? price.discountPrice) || 0;
  ...
  const handleSortOrderBlur = async (id: string) => {
    try {
      ...
      if (res?.success) {
        setCombos(prev => prev.map(c => c.id === id ? { ...c, sortOrder: editingValue } : c));
  ...
  const openCreate = () => {
    setIsEditing(false); setSelectedId(null); setName(""); setHscode("");
    setPrepTime(""); setDescription(""); setCategoryId(""); setSubMenuId("");
    setKotType("KITCHEN"); setSortOrder(0); setImageFile(null);
    setPrice({}); setComboItems([]); setStockConsumption([]);
    setIsPanelOpen(true);
  };

  const openEdit = (c: ComboOffer) => {
    setIsEditing(true); setSelectedId(c.id); setName(c.name);
    setHscode(c.hscode || ""); setPrepTime(c.preparationTime?.toString() || "");
    setDescription(c.description || ""); setCategoryId(c.categoryId);
    setSubMenuId(c.subMenuId || ""); setKotType(c.kotType);
    setSortOrder((c as any).sortOrder ?? 0); setImageFile(c.image?.[0] || null);
    setPrice(c.price || {});
    setComboItems(c.items?.map(i => ({ dishId: i.dishId, quantity: i.quantity, unitPrice: i.unitPrice })) || []);
    setStockConsumption(c.stocks?.map(s => ({ stockId: s.stockId, quantity: s.quantity })) || []);
    setIsPanelOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = isEditing && selectedId 
    ...
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">Manage meal sets and special offers</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard title="Available" value={combos.filter(c => c.isAvailable).length} />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-gray-600 whitespace-nowrap">
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((c) => (
                <tr key={c.id} onClick={() => openEdit(c)} className="hover:bg-red-50/50 transition-colors cursor-pointer group">
                  <td className="px-6 py-4" onClick={(e) => { e.stopPropagation(); setEditingRowId(c.id); setEditingValue((c as any).sortOrder || 0); }}>
                    {editingRowId === c.id ? (
                      <input
                        type="number"
                        value={editingValue}
                        ...
                        onKeyDown={(e) => e.key === 'Enter' && handleSortOrderBlur(c.id)}
                        className="w-16 px-2 py-1 text-sm border border-gray-300 rounded outline-none font-mono"
                        autoFocus
                        onClick={(e) => e.stopPropagation()}
                      />
                    ) : (
                      <span className="text-gray-400 font-mono hover:text-gray-900 cursor-text">{(c as any).sortOrder ?? 0}</span>
                    )}
                  </td>
                  <td className="px-6 py-4 font-medium text-gray-900 flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-gray-100 border border-gray-200 overflow-hidden shrink-0">
                      {c.image?.[0] ? <img src={c.image[0]} className="w-full h-full object-cover" /> : 
                      <div className="w-full h-full flex items-center justify-center bg-slate-100"><Gift size={16} className="text-gray-400" /></div>}
                    </div>
                    {c.name}
                  </td>
                  <td className="px-6 py-4">{categories.find(cat => cat.id === c.categoryId)?.name || "-"}</td>
                  <td className="px-6 py-4 font-bold text-gray-900">${c.price?.listedPrice || 0}</td>
      ...
      <SidePanel isOpen={isPanelOpen} onClose={() => setIsPanelOpen(false)} title={isEditing ? "Edit Combo" : "New Combo"}>
        <div className="space-y-8 pb-24">
          <section className="space-y-4">
            <h3 className="text-sm font-bold text-gray-900 uppercase border-b pb-2">Details</h3>
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2 sm:col-span-1">
                <label className="text-sm font-semibold text-gray-700 block mb-2">Combo Name *</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:border-red-500 outline-none" value={subMenuId} onChange={(e) => setSubMenuId(e.target.value)}>
                  <option value="">Select Sub Menu</option>
                  {subMenus.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
              </div>
            </div>
            <ImageUpload label="Combo Image" value={typeof imageFile === "string" ? imageFile : undefined} onChange={setImageFile} />
          </section>

          <section className="space-y-4">
            <h3 className="text-sm font-bold text-gray-900 uppercase border-b pb-2">Items in Combo</h3>
              ...
              {comboItems.map((item, index) => (
                <div key={index} className="flex gap-2 items-center bg-gray-50 p-2 rounded-lg border">
                  ...
                  >
                    ...
                    {dishes.map(d => <option key={d.id} value={d.id}>{d.name} (${d.price?.listedPrice})</option>)}
                  </select>
                  <input 
                    type="number" 
                  ...
                  />
                  <button onClick={() => setComboItems(comboItems.filter((_, i) => i !== index))} className="text-red-500 hover:bg-red-50 p-1 rounded"><Trash2 size={16} /></button>
                </div>
              ))}
              <Button variant="secondary" className="w-full text-xs" onClick={() => setComboItems([...comboItems, { dishId: "", quantity: 1, unitPrice: 0 }])}>
          ...
          <section className="space-y-4">
            <h3 className="text-sm font-bold text-gray-900 uppercase border-b pb-2">Pricing</h3>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 bg-white border-t flex items-center gap-3 z-10">
          {isEditing && selectedId && (
            <Button onClick={async () => { if (!confirm(`Delete combo?`)) return; try { const res = await deleteCombo(selectedId); if (res?.success) { refresh(); setIsPanelOpen(false); router.refresh(); } } catch (e) { toast.error("Failed"); } }} variant="secondary" className="bg-red-50 text-red-600 border-red-200 hover:bg-red-100"><Trash2 size={18} /></Button>
          )}
          ...
          <Button onClick={handleSubmit} className="flex-1 bg-red-600 text-white shadow-lg" disabled={!name || isSaving || uploading}>
            {isSaving || uploading ? "Saving..." : isEditing ? "Update Combo" : "Create Combo"}
          </Button>
        </div>
      </SidePanel>
    </div>
